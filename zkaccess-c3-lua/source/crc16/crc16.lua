...
function M.crc16_byte(byte, crc)
    ...
    -- print("bit.bxor(crc, byte) crc16_byte : ", bit.bxor(crc, byte))
    local crc_div = calc_divisor(bit.bxor(crc, byte))
    -- print("crc_div crc16_byte : ", crc_div)
...
function M.crc16_byte_array(byte_array, crc)
    ...
    -- print("crc16_byte_array : " .. table.concat(byte_array, "; "))

    for i, byte in ipairs(byte_array) do
        if type(byte) == 'string' then
            crc = M_crc16_string(byte, crc)
        else
            -- print("before crc16_byte_array " .. tostring(i) .. " : " .. tostring(byte) .. " => " .. tostring(crc))

            crc = M_crc16_byte(byte, crc)
            -- print("after crc16_byte_array " .. tostring(i) .. " : " .. tostring(byte) .. " => " .. tostring(crc))
            -- print("***************************************")
        end
    end
...
function M.crc16(data, crc)
    ...
    elseif type(data) == 'table' then
        -- print("Table ==> M_crc16_byte_array")
