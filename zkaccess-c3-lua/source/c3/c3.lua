...
local function M_get_message_header(data_arr)
    ...
    if #data_arr >= 5 then
        if data_arr[1] == consts.C3_MESSAGE_START and data_arr[2] == consts.C3_PROTOCOL_VERSION then
...
local function M_sock_send_data(command, data)
    ...
    print("GAYL => M_sock_send_data : " .. message_length)

    print("lsb(message_length) : " .. tostring(CRC.lsb(message_length)))
    print("msb(message_length) : " .. tostring(CRC.msb(message_length)))

    local message = {consts.C3_PROTOCOL_VERSION, command.request or 0x00, CRC.lsb(message_length),
                     CRC.msb(message_length), sessionID[2] or 0x00, -- LSB of sessionId first
    sessionID[1] or 0x00, -- MSB of sessionId second
    CRC.lsb(requestNr), CRC.msb(requestNr)}

    print("Before message : " .. table.concat(message, ", "))
    print("Type message : " .. type(message))
    -- Append data bytes (Payload)
    if data then
    ...
    print("after Last message with data : " .. table.concat(message, ", "))

    local checksum = CRC.crc16(message)
    print("checksum : " .. checksum)
    table.insert(message, CRC.lsb(checksum))
    print("CRC.lsb(checksum) : " .. CRC.lsb(checksum))
    table.insert(message, CRC.msb(checksum))
    print("CRC.msb(checksum) : " .. CRC.msb(checksum))

    table.insert(message, 1, consts.C3_MESSAGE_START)
    table.insert(message, consts.C3_MESSAGE_END)
    print("Last message : " .. table.concat(message, ", "))
    print("utils.arr_to_str(message) : " .. utils.arr_to_str(message))
...
local function M_sock_receive_data(expected_command)
    ...
    if ok then
        ...
        print(string.format("Receiving data failed (incomplete): %s (sessionId: %d)", header_str,
                  utils.bytes_to_num(sessionID)))
    end
...
local function M_sock_send_receive_data(command, send_data, callback)
    ...
    local cb = nil
    if callback then
        -- print("M_sock_send_receive_data_cb_wrapper : ")
        cb = M_sock_send_receive_data_cb_wrapper(callback)
    end

    -- print("command value: " .. command.request)
    -- print("str_to_arr (M_sock_send_receive_data): ")
    -- print(send_data)
...
local function M_connect_to_C3_cb(size, data_arr)
    ...
    if size == 4 then
        ...
        print("sessionID[1] : " .. tostring(sessionID[1]))
    end
...
function M.getDeviceParameters(params_arr)
    ...
    return M_sock_send_receive_data(consts.C3_COMMAND_GETPARAM, utils.str_to_arr(param_data), M_getDeviceParameters_cb)
end

function M.controlDevice(control_command_object)
    assert(connected)

    M_sock_send_receive_data(consts.C3_COMMAND_CONTROL, control_command_object.to_byte_array())
end
