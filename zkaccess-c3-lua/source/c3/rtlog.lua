...
function RTLog.DAStatusRecord()
    ...
    function self.from_byte_array(data_arr, from_idx)
        ...
        self.time_second = utils.byte_array_to_time({data_arr[from_idx + 15], data_arr[from_idx + 14],
                                                     data_arr[from_idx + 13], data_arr[from_idx + 12]})
        ...
        for key, value in pairs(self) do
            if type(value) ~= 'function' then
                ...
                elseif key == "event_type" then
                    table.insert(res_arr, string.format("%-12s %-10s %s", key, tostring(value),
                        tostring(consts.C3_EVENT_TYPE[value])))
                elseif key == "verified" then
                    table.insert(res_arr, string.format("%-12s %-10s %s", key, tostring(value),
                        consts.C3_VERIFIED_MODE[value or 0]))
                elseif key == "alarm_status" then
                    ...
                    for i, v in ipairs(value) do
                        table.insert(res_arr, string.format("    Door %-2s %-4s %s", i, v,
                            concat_if_in_bitset(v, consts.C3_ALARM_STATUS)))
                    end
                elseif key == "dss_status" then
                    table.insert(res_arr, string.format("%-12s", key))
                    for i, v in ipairs(value) do
                        table.insert(res_arr, string.format("    Door %-2s %-4s %s", i, v,
                            tostring(consts.C3_DSS_STATUS[v or 0])))
                    end
                else
...
function RTLog.EventRecord()
    ...
    function self.from_byte_array(data_arr, from_idx)
        self.card_no = utils.bytes_to_num({data_arr[from_idx + 3], data_arr[from_idx + 2], data_arr[from_idx + 1],
                                           data_arr[from_idx + 0]})
        self.pin = utils.bytes_to_num({data_arr[from_idx + 7], data_arr[from_idx + 6], data_arr[from_idx + 5],
                                       data_arr[from_idx + 4]})
        self.verified = data_arr[from_idx + 8]
        ...
        for key, value in pairs(self) do
            if type(value) ~= 'function' then
                ...
                elseif key == "in_out_state" then
                    table.insert(res_arr,
                        string.format("%-12s %-10s %s", key, tostring(value), consts.C3_INOUT_STATUS[value]))
                elseif key == "event_type" then
                    table.insert(res_arr, string.format("%-12s %-10s %s", key, tostring(value),
                        tostring(consts.C3_EVENT_TYPE[value])))
                elseif key == "verified" then
